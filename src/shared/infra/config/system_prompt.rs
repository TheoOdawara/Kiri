...
const SYSTEM_PROMPT_TEMPLATE: &str = concat!(
    ...
    "- auto — calls run without prompting (the user still sees every call as it executes). The ",
    "  exceptions still prompt: deleting or moving a path, and a shell command classified as ",
    "  irreversible (rm, git push, a publish, sudo, inline code via python3 -c / node -e). Prefer the ",
    "  reversible form so the user is not interrupted.\n",
    ...
    "  installs are not), and every command is confirmed live. Do NOT edit files while ",
