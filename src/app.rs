...
/// A failed open or init degrades to an inert in-memory store rather than aborting. The warning is
/// returned, not printed, so the caller surfaces it on its own channel and a lazy `/sync` cannot corrupt
/// the live TUI. Opening a second SQLite handle to the memory tools' file is safe.
